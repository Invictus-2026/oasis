...
function AdHocUpload() {
  ...
  return (
    <div className="mt-6 flex flex-col gap-6">
      ...
      {/* Upload Controls */}
      <div className="bg-white dark:bg-ink-100 rounded-xl border border-ink-200 shadow-sm p-5">
         <div className="flex flex-col gap-4">
            ...
            {file && (
               <div className="flex items-center gap-3 bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/30 p-3 rounded-lg text-blue-800 dark:text-blue-300 text-sm">
                  <FileImage className="w-5 h-5 text-blue-500 dark:text-blue-400 shrink-0" />
            ...
            {error && (
               <div className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded p-3 mt-1">
      ...
      {/* Canvas View — always dark chrome, like a photo viewer, regardless of app theme */}
      {imgUrl && (
         <div className="bg-[#020617] rounded-xl overflow-hidden shadow-inner border border-[#1e293b] flex justify-center p-4">
            <canvas ref={canvasRef} className="max-w-full h-auto bg-[#0f172a] rounded" />
         </div>
      )}
            ...
            <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-xl p-5 flex flex-col md:flex-row items-center justify-between gap-6">
               <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-lg font-black text-amber-900 dark:text-amber-300 tracking-tight">
               ...
               <div className="flex items-center gap-6 text-center bg-white dark:bg-ink-100 p-4 rounded-lg border border-amber-100 dark:border-amber-500/20 shadow-sm min-w-[200px] justify-center">
                  <div>
                     <div className="text-3xl font-black text-amber-600 dark:text-amber-400 tabular-nums">{result.total_area_km2.toFixed(2)}</div>
                     <div className="text-[10px] font-bold uppercase tracking-widest text-amber-500 dark:text-amber-400">Total Area (km²)</div>
                  </div>
               </div>
                  ...
                  <div className="space-y-3">
                    {result.oil_regions.map((r, i) => (
                       <div key={i} className="flex gap-4 p-4 rounded-lg border border-ink-200 bg-ink-50">
                          <div className="w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 flex items-center justify-center font-bold text-xs shrink-0">#{i + 1}</div>
      ...
      <div className="page-shell">
        ...
        <header className="page-header flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          <div className="flex bg-ink-100 p-1 rounded-lg self-start border border-ink-200">
             <button onClick={() => setMode("case")} className={`px-4 py-1.5 rounded-md text-xs uppercase tracking-wider font-bold transition-colors ${mode === "case" ? "bg-white dark:bg-ink-200 shadow-sm text-blue-600 dark:text-blue-400" : "text-ink-500 hover:text-ink-700"}`}>Case Study</button>
             <button onClick={() => setMode("upload")} className={`px-4 py-1.5 rounded-md text-xs uppercase tracking-wider font-bold transition-colors ${mode === "upload" ? "bg-white dark:bg-ink-200 shadow-sm text-blue-600 dark:text-blue-400" : "text-ink-500 hover:text-ink-700"}`}>Custom Upload</button>
          </div>
        </header>
