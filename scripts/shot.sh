...
w, h = im.size
MAP_BG = (10, 21, 38)     # the style's background-color
...
if ok == 0:
    ...
    sys.exit(1)
if not any(near(s, MAP_BG) for s in samples):
    print(f"  \033[33m!\033[0m map painted but background is unexpected: {samples[:2]}")
else:
    print(f"  \033[32m✓\033[0m map canvas painted ({ok}/{len(samples)} samples non-blank)")
